...
2. Identifiant et clé d'accès
   ...
   La clé nommée par ~key_name~ doit exister avec la portée «auth».
...
then
	echo -n "https://progression.dti.crosemont.quebec/api/v3"
else
...
# Inscription
/usr/bin/curl "<<get_api_url()>>/user" -X PUT --data '{"username": "jdoe", "password": "Crosemont2021!"}' -H "Content-Type: application/json" | grep -Po '(?<=Token":").*(?=")'

# Récupère le token
export TOKEN=$(/usr/bin/curl "<<get_api_url()>>/user/jdoe/tokens" --data '{"identifiant": "jdoe", "password": "Crosemont2021!", "ressources": ["tout": {"url": ".*", "method": ".*"}] }' -H "Content-Type: application/json" | grep -Po '(?<=Token":").*(?=")')
...
curl "<<get_api_url()>>/user/jdoe" -X PUT --data '{"identifiant": "jdoe", "courriel": "jdoe@courriel.com", "password": "Crosemont2021!"}' -H "Content-Type: application/json"
#+END_SRC
...
#+NAME: AUTHENTIFICATION
#+BEGIN_SRC bash :eval yes
curl "<<get_api_url()>>/user/jdoe/tokens" --data '{"identifiant": "jdoe@courriel.com", "password": "Crosemont2021!", "ressources": ["tout": {"url": ".*", "method": ".*"}] }' -H "Content-Type: application/json"
#+END_SRC
