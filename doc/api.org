...
export TIMESTAMP=$(echo $RES_TENTATIVE | grep -Po '(?<="date_soumission": )\d*')
#+END_SRC
...
RES_CLE=$(/usr/bin/curl --data "$DATA" "https://progression.dti.crosemont.quebec/dev/api/v1/user/jdoe/cles" -H "Content-Type: application/json" -H "Authorization: Bearer $TOKEN")

export NOM_CLE=$(echo $RES_CLE | grep -Po '(?<="id":"jdoe\\/)[^"]*')
export SECRET=$(echo $RES_CLE | grep -Po '(?<="secret":")\w*')
#+END_SRC
