...
Les ressources sont définies par leur URI et par la méthode de requête HTTP, avec une expression régulière. Un token ressource peut décrire une ou plusieurs ressources. Il permet d'accéder aux ressoures d'un autre utilisateur.
