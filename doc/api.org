...
Génère un token JWT qui définit l'accès à des ressources en fonction de leur méthode de leur url.
...
#+BEGIN_SRC bash :eval yes :session *bash* :results output :exports both :post json_pretty(*this*)
curl -X POST "<<get_api_url()>>/token/jdoe" -H "Authorization: Bearer $TOKEN" -d ressources
#+END_SRC
