...
| Nom                                   | Cardinalité | Type       | description                                                   |
|---------------------------------------+-------------+------------+---------------------------------------------------------------|
| avancement                            | 1-1         | Avancement | avancement pour cette question pour l'utilisateur courant     |
| ébauches                              | 1-n         | Solution   | liste d'ébauches de Solution                                  |
| tests                                 | 1-n         | Test       | liste de Tests                                                |
| accessibilité (relation? lien? meta?) | 1-1         | bool       | Vrai si la question est accessible pour l'utilisateur courant |

Liens :
| Nom           | Description                                                   |
|---------------+---------------------------------------------------------------|
| suivante      | Question suivante pour l'utilisateur courant                  |
...
| Classe    | Identifiant              | Propriétés      | Type | Valeurs   |
|-----------+--------------------------+-----------------+------+-----------|
| Tentative | question/date_soumission | date_soumission | int  | timestamp |
|           |                          | feedback        | str  |           |
...
| Classe   | Identifiant     | Propriétés      | Type | Valeurs |
|----------+-----------------+-----------------+------+---------|
| Résultat | question/numéro | numéro          | int  |         |
|          |                 | sortie_observée | str  |         |
|          |                 | sortie_erreur   | str  |         |
|          |                 | résultat        | bool |         |
|          |                 | feedback        | str  |         |
...
#+NAME: json_pretty
#+begin_src sh :eval yes :var json="" :results output :exports none
echo $json|python -m json.tool
#+end_src


** Connexion en tant qu'utilisateur ~jdoe~ :
...
#+RESULTS:
: {"token":"eyJ0eXAiOiJKV1QiLCJhbGciOiJIUzI1NiJ9.eyJ1c2VyIjp7InVzZXJuYW1lIjoiamRvZSIsInJvbGUiOjAsImlkIjo3LCJhY3RpZiI6bnVsbH0sImN1cnJlbnQiOjE2MTMwNjk1MDMsImV4cGlyZWQiOjE2MTMxNTU5MDN9.9i4E8TZHl5MTfxuDOmAapzNgQVwd1Um1-ueAozL1TMU"}

** Obtenir le profil de l'utilisateur connecté ~jdoe~ et ses avancements :

#+BEGIN_SRC sh :var :eval no TOKEN=get_token :results output
...
: {
:   "links": {
:     "self": "https://progression.dti.crosemont.quebec/api/v1/user"
:     },
:   "data" : [ {
:     "type" : "User",
:     "id" : "jdoe"
...
:       "catégories": {
:         "links": {
:           "self": "https://progression.dti.crosemont.quebec/api/v1/user/jdoe/relationships/catégories"
:           "related": "https://progression.dti.crosemont.quebec/api/v1/user/jdoe/catégories"
:         },
:         "data": [
:           { "type": "Catégorie", "id": "les_boucles" }
:           { "type": "Catégorie", "id": "les_fonctions" }
:         }
:       },
:       "avancements": {
:         "links": {
:           "self": "https://progression.dti.crosemont.quebec/api/v1/user/jdoe/relationships/avancements"
:           "related": "https://progression.dti.crosemont.quebec/api/v1/user/jdoe/avancements"
:         },
:         "data": [
:           { "type": "Avancement", "id": "les_fonctions/appeler_une_fonction" }
:           { "type": "Avancement", "id": "les_fonctions/appeler_une_autre_fonction" }
:         }
:       }
:     }],
:   "included": {
:     "avancements": [
:       "type" : "Avancement",
:       "id" : "les_fonctions/appeler_une_fonction",
:       "attributes" : {
:         "état": 1
:       },
:       "relationships" : {
...
:           "links" :
:           "data" : [
:             "type" : "Solution",: "id" : 1234567
:             "type" : "Solution",: "id" : 1234987
:           ]
:         ]
:       },
:       "type" : "Avancement",
:       "id" : "programmation_1/les_fonctions/appeler_une_autre_fonction"
:       "attributes" : {
:         "état": 2
:       },
:       "relationships" : {
...
** Obtenir la liste des catégories disponibles pour l'utilisateur ~jdoe~ :
...
** Obtenir la sous-catégorie «les_fonctions» :
...
** Obtenir la question «appeler_une_fonction_paramétrée» :

#+BEGIN_SRC sh :eval yes :var TOKEN=get_token :post json_pretty(*this*) :results output
curl https://progression.dti.crosemont.quebec/api/v1/question/2 -H "Authorization: Bearer $TOKEN"
#+END_SRC
...
** Obtenir l'ébauche de solution en Python pour la question «appeler_une_fonction» :
...
** Obtenir une solution soumise pour la question «appeler_une_fonction» :
