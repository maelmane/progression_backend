...
| POST    | ~/auth~   | Authentification         | identifiant (username ou courriel) et password | token, un token JWT |
|         |           |                          | username, key_name et key_secret               | token, un token JWT |
| PUT     | ~/user~   | Inscription              | user                                           | User                |
...
|-----------------+------+--------------------------------------------------------------|
| nom             | str  |                                                              |
| sortie_attendue | str  | La sortie attendue, ou chaîne vide si sortie_cachée est vrai |
| feedback_pos    | str  |                                                              |
| feedback_neg    | str  |                                                              |
| sortie_cachée   | bool | vrai si la sortie attendue a été remplacée par ""            |
...
| expiration  | int  | timestamp d'expiration                 |
| data        | str  | données incluses dans le token         |
| permissions | str  | permissions incluses dans le token     |
| jwt         | str  | token jwt signé                        |
| version     | int  | version de l'api ayant généré le token |

**** Data

data contient un objet peuplé de n'importes quelles données utiles.
...
| username    | str  |                                                  |
| courriel    | str  |                                                  |
| état        | int  | 0=inactif, 1=actif, 2=en attente de confirmation |
...
export TOKEN=$(/usr/bin/curl "<<get_api_url()>>/auth" --data '{"identifiant": "jdoe", "password": "Crosemont2021!"}' -H "Content-Type: application/json" | grep -Po '(?<=Token":").*(?=")')

if [ -z "$TOKEN" ]
then
    export TOKEN=$(/usr/bin/curl "<<get_api_url()>>/user" -X PUT --data '{"username": "jdoe", "password": "Crosemont2021!"}' -H "Content-Type: application/json" | grep -Po '(?<=Token":").*(?=")')
fi
...
curl "<<get_api_url()>>/auth" --data '{"identifiant": "jdoe", "password": "Crosemont2021!"}' -H "Content-Type: application/json"
#+END_SRC

#+RESULTS:
** Authentification par courriel en tant qu’utilisateur ~jdoe~

#+NAME: AUTHENTIFICATION
#+BEGIN_SRC bash :eval yes
curl "<<get_api_url()>>/auth" --data '{"identifiant": "jdoe@courriel.com", "password": "Crosemont2021!"}' -H "Content-Type: application/json"
#+END_SRC
