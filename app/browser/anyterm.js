...
function open_term(r,c,p,charset,sb) {
  rows = r;
  cols = c;
  scrollback = sb;
...
function create_button(label,fn,class_name) {
  var button=document.createElement("A");
  var button_t=document.createTextNode(label);
  button.appendChild(button_t);
  if (class_name) button.className=class_name;
  button.onclick=fn;
  return button;
}
...
function create_term(elem_id,title,rows,cols,p,charset,scrollback) {
  ...
  title_p.appendChild(create_button("X",close_term,"boxed right"));
  ...
  var resizer_div = create_resizer();
  frame.appendChild(resizer_div);
...
function create_resizer() {
  ...
  r.className="resizer";
  r.onmousedown=resize_start;

  var i = document.createElement("IMG");
  var src_attr = document.createAttribute("SRC");
  src_attr.value = "resizer.png";
  i.setAttributeNode(src_attr);
...
var resizing = false;
var resize_offs_x;  // Position of mouse relative to bottom right of 
var resize_offs_y;  // frame at start of resizing.

function resize_start(ev) {
  ev.preventDefault();
  resizing = true;
  resize_offs_x = ev.clientX - (frame.clientLeft + frame.clientWidth);
  resize_offs_y = ev.clientY - (frame.clientTop + frame.clientHeight);
  document.onmousemove = resize_move;
  document.onmouseup = resize_end;
}

function resize_move(ev) {
  if (!resizing) return;

  ev.preventDefault();

  var ox = ev.clientX - (frame.clientLeft + frame.clientWidth);
  var oy = ev.clientY - (frame.clientTop + frame.clientHeight);

  var dc = 0;
  var dr = 0;
  var th = 10;
  if (ox < resize_offs_x - th) dc = -1;
  if (ox > resize_offs_x + th) dc =  1;
  if (oy < resize_offs_y - th) dr = -1;
  if (oy > resize_offs_y + th) dr =  1;

  // The resizing doesn't necesarily keep up with the mouse; we could resize 
  // again after receiving the resized screen, or we could adjust the size by 
  // more than one each time.  But this seems OK in practice.

  if (dc != 0 || dr != 0) {
    resize_to(rows+dr,cols+dc);
  }
}

function resize_end(ev) {
  resizing = false;
  document.onmousemove = null;
  document.onmouseup = null;;
}

function resize_to(r,c) {
  if (r < 4 || c < 10) return;
  var resp = sync_load(url_prefix+"anyterm-module","a=resize&s="+session+"&rows="+r+"&cols="+c+cachebust());
  rows = r;
  cols = c;
  handle_resp_error(resp);
  visible_height_frac=Number(rows)/(Number(rows)+Number(scrollback));
}
