...
if [ -z "$test_simple" ]
then
	$DIR/../vendor/bin/phpunit -d memory_limit=-1 --configuration $DIR/../phpunit.xml --coverage-text --colors=never --coverage-cobertura=reports/cobertura-coverage.xml || exit 1
else
