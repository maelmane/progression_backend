...
use progression\domaine\interacteur\{
    ObtenirAvancementProgInt,
    ObtenirSérieInt,
    ObtenirQuestionProgInt,
    FormaterMDInt,
    ExécuterProgInt,
    TraiterRésultatsProgInt,
    PréparerProgInt};
...
class QuestionProgCtl extends ProgCtl
{
    ...
    protected function get_question()
    {
        ...
        $question->enonce = QuestionProgCtl::formaterMD( $question->enonce );
        $question->feedback_pos = QuestionProgCtl::formaterMD( $question->feedback_pos );
        $question->feedback_neg = QuestionProgCtl::formaterMD( $question->feedback_neg );

        foreach ($question->tests as $test) {
            $test->feedback_pos = QuestionProgCtl::formaterMD( $test->feedback_pos );
            $test->feedback_neg = QuestionProgCtl::formaterMD( $test->feedback_neg );
        }
    ...
    private function formaterMD($md){
