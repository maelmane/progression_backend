...
class UserDAO extends EntiteDAO{
	...
	protected function load( $objet ){
		$query=$GLOBALS[ "conn" ]->prepare( 'SELECT userID, username, role FROM users WHERE userID = ? ' );
	...
	public function save( $objet ){
		$query=$GLOBALS[ "conn" ]->prepare( 'INSERT INTO users( username, role ) VALUES ( ?, ? ) ON DUPLICATE KEY UPDATE role=VALUES( role )' );
