...
class QuestionProgEvalDAO extends QuestionDAO{
    ...
    protected static function load( $objet ){
		parent::load( $objet );
		$query=QuestionProgEvalDAO::$conn->prepare( 'SELECT question_prog.lang, 
                                                theme.lang, 
                                                question_prog.setup, 
    ...
    public static function save( $objet ){
		if( !$objet->id ){
			...
			$query=QuestionProgEvalDAO::$conn->prepare( "INSERT INTO question_prog( questionID,
                                                    lang,
                                                    setup,
                                                    pre_exec,
                                                    pre_code,
			...
			$qid=parent::save( $objet );
			$query=QuestionProgEvalDAO::$conn->prepare( "UPDATE question_prog SET lang=?,
                                                    setup=?,
                                                    pre_exec=?,
                                                    pre_code=?,
                                                    in_code=?,
